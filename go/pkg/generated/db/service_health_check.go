...
const insertServiceHealthCheckQuery = "insert into `service_health_check` (`uuid`,`fq_name`,`http_method`,`delayUsecs`,`delay`,`url_path`,`timeoutUsecs`,`expected_codes`,`health_check_type`,`max_retries`,`timeout`,`monitor_type`,`enabled`,`creator`,`user_visible`,`last_modified`,`group_access`,`owner`,`owner_access`,`other_access`,`group`,`enable`,`description`,`created`,`display_name`,`key_value_pair`,`perms2_owner_access`,`global_access`,`share`,`perms2_owner`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
const updateServiceHealthCheckQuery = "update `service_health_check` set `uuid` = ?,`fq_name` = ?,`http_method` = ?,`delayUsecs` = ?,`delay` = ?,`url_path` = ?,`timeoutUsecs` = ?,`expected_codes` = ?,`health_check_type` = ?,`max_retries` = ?,`timeout` = ?,`monitor_type` = ?,`enabled` = ?,`creator` = ?,`user_visible` = ?,`last_modified` = ?,`group_access` = ?,`owner` = ?,`owner_access` = ?,`other_access` = ?,`group` = ?,`enable` = ?,`description` = ?,`created` = ?,`display_name` = ?,`key_value_pair` = ?,`perms2_owner_access` = ?,`global_access` = ?,`share` = ?,`perms2_owner` = ?;"
const deleteServiceHealthCheckQuery = "delete from `service_health_check` where uuid = ?"
const listServiceHealthCheckQuery = "select `uuid`,`fq_name`,`http_method`,`delayUsecs`,`delay`,`url_path`,`timeoutUsecs`,`expected_codes`,`health_check_type`,`max_retries`,`timeout`,`monitor_type`,`enabled`,`creator`,`user_visible`,`last_modified`,`group_access`,`owner`,`owner_access`,`other_access`,`group`,`enable`,`description`,`created`,`display_name`,`key_value_pair`,`perms2_owner_access`,`global_access`,`share`,`perms2_owner` from `service_health_check`"
const showServiceHealthCheckQuery = "select `uuid`,`fq_name`,`http_method`,`delayUsecs`,`delay`,`url_path`,`timeoutUsecs`,`expected_codes`,`health_check_type`,`max_retries`,`timeout`,`monitor_type`,`enabled`,`creator`,`user_visible`,`last_modified`,`group_access`,`owner`,`owner_access`,`other_access`,`group`,`enable`,`description`,`created`,`display_name`,`key_value_pair`,`perms2_owner_access`,`global_access`,`share`,`perms2_owner` from `service_health_check` where uuid = ?"
...
func createServiceHealthCheckWhereQuery(where map[string]interface{}) (string, []interface{}) {
	if where == nil {
		return "", nil
	}
	results := []string{}
...
func ListServiceHealthCheck(tx *sql.Tx, where map[string]interface{}, offset int, limit int) ([]*models.ServiceHealthCheck, error) {
	...
	whereQuery, values := createServiceHealthCheckWhereQuery(where)
	var rows *sql.Rows
	var err error
