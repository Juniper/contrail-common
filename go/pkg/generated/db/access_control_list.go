...
const insertAccessControlListQuery = "insert into `access_control_list` (`fq_name`,`owner_access`,`other_access`,`group`,`group_access`,`owner`,`enable`,`description`,`created`,`creator`,`user_visible`,`last_modified`,`display_name`,`key_value_pair`,`share`,`perms2_owner`,`perms2_owner_access`,`global_access`,`access_control_list_hash`,`dynamic`,`acl_rule`,`uuid`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
const updateAccessControlListQuery = "update `access_control_list` set `fq_name` = ?,`owner_access` = ?,`other_access` = ?,`group` = ?,`group_access` = ?,`owner` = ?,`enable` = ?,`description` = ?,`created` = ?,`creator` = ?,`user_visible` = ?,`last_modified` = ?,`display_name` = ?,`key_value_pair` = ?,`share` = ?,`perms2_owner` = ?,`perms2_owner_access` = ?,`global_access` = ?,`access_control_list_hash` = ?,`dynamic` = ?,`acl_rule` = ?,`uuid` = ?;"
const deleteAccessControlListQuery = "delete from `access_control_list` where uuid = ?"
const listAccessControlListQuery = "select `fq_name`,`owner_access`,`other_access`,`group`,`group_access`,`owner`,`enable`,`description`,`created`,`creator`,`user_visible`,`last_modified`,`display_name`,`key_value_pair`,`share`,`perms2_owner`,`perms2_owner_access`,`global_access`,`access_control_list_hash`,`dynamic`,`acl_rule`,`uuid` from `access_control_list`"
const showAccessControlListQuery = "select `fq_name`,`owner_access`,`other_access`,`group`,`group_access`,`owner`,`enable`,`description`,`created`,`creator`,`user_visible`,`last_modified`,`display_name`,`key_value_pair`,`share`,`perms2_owner`,`perms2_owner_access`,`global_access`,`access_control_list_hash`,`dynamic`,`acl_rule`,`uuid` from `access_control_list` where uuid = ?"
...
func createAccessControlListWhereQuery(where map[string]interface{}) (string, []interface{}) {
	if where == nil {
		return "", nil
	}
	results := []string{}
...
func ListAccessControlList(tx *sql.Tx, where map[string]interface{}, offset int, limit int) ([]*models.AccessControlList, error) {
	...
	whereQuery, values := createAccessControlListWhereQuery(where)
	var rows *sql.Rows
	var err error
