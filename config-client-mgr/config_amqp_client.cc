...
bool ConfigAmqpClient::ProcessMessage(const string &json_message) {
    ...
    if (document.HasParseError()) {
        ...
        if ((oper == "CREATE") || (oper == "UPDATE")) {
            ...
            // It is possible (as seen with bug #1787511) that RabbitMQ
            // might club the CREATE and UPDATE for an object if they
            // happen in quick succession (for instance control node
            // failover). In such cases, we could only get an UPDATE of
            // the object without a preceding CREATE. Handle the UPDATE
            // as a CREATE and add object to FQNameCache.
