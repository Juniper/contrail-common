 ...
 public:
    ...
    class UUIDCacheEntry {
     public:
        ...
        //void EnableCassandraReadRetry(const string uuid);
        //void DisableCassandraReadRetry(const string uuid);
     ...
     private:
        //bool CassReadRetryTimerExpired(const string uuid);
...
class ConfigEtcdClient : public ConfigDbClient {
 public:
    ...
    // FQName functions are not applicable to ETCD since
    // there is no FQName table. Kept here only since
    // ConfigDbClient defines them as pure virtual functions.
    // If and when Cassandra support is taken away, these
    ...
    // This is again not needed for ETCD. Kept since ConfigJsonParser
    // uses this for Cassandra. For ETCD, this will return false
    // since there are no property lists or maps. Those have been
    // converted to property objects/arrays already in ETCD.
