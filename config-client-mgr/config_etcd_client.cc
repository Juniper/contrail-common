...
void ConfigEtcdPartition::AddUUIDToProcessList(const string &oper,
    ...
    if (ret.second) {
        ...
        /**
          ...
          * If operation is DELETE and was preceeded by
          * CREATE, remove from uuid_process_set_.
          * For other cases (CREATE/UPDATE) replace the
          * entry with the new value and oper.
          */
        if ((oper == "DELETE") &&
