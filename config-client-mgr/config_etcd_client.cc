...
void ConfigEtcdPartition::AddUUIDToProcessList(const string &oper,
    ...
    if (ret.second) {
        /**
          * UUID not resent in uuid_process_set_.
...
void ConfigEtcdPartition::GenerateAndPushJson(const string &uuid,
    ...
    bool bgpaas_sess_attr_processed = false;
    ...
    while (itr != doc.MemberEnd()) {
        ...
        /**
          ...
          */
        if (key.compare("bgpaas_session_attributes") == 0 &&
            !bgpaas_sess_attr_processed) {
            // To make sure that bgpaas_session_attributes
            // is processed only once since we re-insert it
            // with a value of null string
            bgpaas_sess_attr_processed = true;
            // Replace value with null string
            itr = doc.EraseMember(itr);
            Value val(key.c_str(), doc.GetAllocator());
            doc.AddMember(val,            // key string
        ...
        if (key.find("_refs") != string::npos) {
            /*
            refString.replace(refString.start(), refString.end(),
                              "null", "{}");
            */
        ...
        if (!deleted && itr != doc.MemberEnd()) itr++;
    }
    ...
    refDoc.Accept(writer);
    string refString = sb.GetString();
    CONFIG_CLIENT_DEBUG(ConfigClientMgrDebug,
