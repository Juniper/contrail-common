                   ...
                   num_workers_(num_workers)
{
    ...
    cc.use_ssl = options.etcd_use_ssl;
        ...
        if (!is_new && cacheDoc.HasMember(key.c_str()) &&
            ...
            if (key.find("_refs") != string::npos &&
                ...
                /**
                  ...
                  * leaving only the ones that still remain in configuartion.
                  */
                FixRefsInUpdate(updDoc, cacheDoc, key);
                ++itr;
            } else {
                // Non-refs.
...
void ConfigEtcdPartition::FixRefsInUpdate(Document &updDoc,
                                        Document &cacheDoc,
                                        const string &key) {
