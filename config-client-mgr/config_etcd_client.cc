...
const string ConfigEtcdPartition::UUIDToFQName(string uuid) const {
    UUIDCacheMap::const_iterator uuid_iter = uuid_cache_map_.find(uuid);
    if (uuid_iter == uuid_cache_map_.end()) {
        return string();
    }
    return uuid_iter->second->GetFQName();
}
    ...
    while (itr != doc.MemberEnd()) {
        ...
        if (key.find("_refs") != string::npos) {
            ...
            for (SizeType i = 0; i < v->Size(); i++) {
                ...
                string ref_uuid = uuidVal.GetString();
                string ref_fq_name = UUIDToFQName(ref_uuid);
