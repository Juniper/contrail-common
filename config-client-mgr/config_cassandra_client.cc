...
bool ConfigCassandraPartition::StoreKeyIfUpdated(const string &uuid,
    ...
    if (adapter->key == "type") {
        ...
        if (context.fq_name.empty()) {
            ...
            // Check if it is a draft object, this is done by checking if the
            // parent name  has "draft" in it.
            ...
            if (pos2 != string::npos) {
                ...
                transform(parent_name.begin(), parent_name.end(),
                        parent_name.begin(), ::tolower);
                if (parent_name.find("draft") != string::npos)
