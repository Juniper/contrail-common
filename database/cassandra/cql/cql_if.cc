 ...
 public:
    ...
    void operator()(const IpAddress &tipaddr, size_t index) const {
        ...
        if (tipaddr.is_v4()) {
            ...
            cinet = cci_->CassInetInitV4(RAW_ARRAY(tv4.to_bytes()));
        } else {
            boost::asio::ip::address_v6 tv6(tipaddr.to_v6());
            cinet = cci_->CassInetInitV6(RAW_ARRAY(tv6.to_bytes()));
        }
    ...
    switch (cvtype) {
      ...
      case CASS_VALUE_TYPE_INET: {
        ...
        if (ctinet.address_length == CASS_INET_V4_LENGTH) {
            ...
            memcpy(RAW_ARRAY(ipv4), ctinet.address, CASS_INET_V4_LENGTH);
            ipaddr = Ip4Address(ipv4);
        } else if (ctinet.address_length == CASS_INET_V6_LENGTH) {
            Ip6Address::bytes_type ipv6;
            memcpy(RAW_ARRAY(ipv6), ctinet.address, CASS_INET_V6_LENGTH);
...
bool CqlIfImpl::ConnectSchemaSync() {
    /* If Connect is called multiple times due to DB failure,
     * then it is better to delete previous session and use
     * a new one to avoid gradual leak.
     */
    schema_session_.reset();
...
bool CqlIfImpl::ConnectSync() {
     ...
     */
    session_.reset();
    impl::CassSessionPtr session(cci_->CassSessionNew(), cci_);
