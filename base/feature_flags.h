 ...
 *     }
 *
 * FlagConfigUveManager class is used to interface with user configuration
 * and send UVE to analytics when configuration changes.
...
class FlagConfig {
public:
    FlagConfig(const std::string &name,const std::string &release,
               bool enabled, bool configured, FlagState::type state,
    ...
    bool operator == (const FlagConfig &rhs) const {
        if (!(name_ == rhs.name_))
            return false;
        if (!(release_ == rhs.release_))
            return false;
        if (!(enabled_ == rhs.enabled_))
            return false;
        if (!(configured_ == rhs.configured_))
            return false;
 ...
 *
 * When user configuration for a flag changes, UVE is sent to analytics.
 * -----------------------------------------------------------------------------
 */
class FlagConfigUveManager {
public:
    static FlagConfigUveManager* GetInstance();

    /**
     * API to set/update user config. Called when run-time updates are
     * received for feature.
     */
    void Set(const std::string &name,const std::string &release_info,
             bool enabled, bool configured, FlagState::type state,
             ContextVec &context_infos);
     ...
     * FlagConfigUveManager. This will be invoked when the FlagConfigUveManager
...
private:
    ...
    // Singleton
    FlagConfigUveManager(FlagManager *manager, FlagUveCb send_uve_cb);
    static void CreateInstance(FlagManager *mgr, FlagUveCb send_uve_cb);

    static boost::scoped_ptr<FlagConfigUveManager> instance_;
...
public:
     ...
     */
    void Set(const std::string &name, const std::string &release, bool enabled,
             bool conf, FlagState::type state, ContextVec &context_infos);
...
private:
    friend class Flag;
    friend class FlagConfigUveManager;
