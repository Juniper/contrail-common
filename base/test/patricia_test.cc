...
TEST_F(PatriciaBaseTest, MC) {
    ...
    route = new Route(0x0a00000b, 32, 1);
    ret = itbl_->Insert(route);
    EXPECT_EQ(ret, true);
    ret = itbl_->Insert(route);
    EXPECT_EQ(ret, false);
    route = new Route(0x0a000000, 24, 2);
    ret = itbl_->Insert(route);
    EXPECT_EQ(ret, true);
    ret = itbl_->Insert(route);
    EXPECT_EQ(ret, false);
    route = new Route(0x0a000001, 32, 3);
    ret = itbl_->Insert(route);
    EXPECT_EQ(ret, true);
    ret = itbl_->Insert(route);
    EXPECT_EQ(ret, false);

    route_next = itbl_->NodeToData(itbl_->root_);
    EXPECT_EQ(route_next->nexthop_, 2);

    route_next = itbl_->GetNext(route_next);
    EXPECT_EQ(route_next->nexthop_, 3);
    
    route_next = itbl_->FindNext(route_next);
    EXPECT_EQ(route_next->nexthop_, 1);
