...
void t_cpp_generator::generate_sandesh_http_reader(ofstream& out,
    ...
    indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Value of query is: \" << snh_query << std::endl);" << endl;
    ...
    scope_up(out);
   // indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Checking when flow comes here at beginning of loop \" << std::endl);" << endl;
    indent(out) << "if (it1 != tokens.end())" << endl;
    scope_up(out);
    indent(out) << "string tok = (*it1++).c_str();" << endl ;
   // indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Value of token is: \" << tok << std::endl);" << endl;
   ...
   // indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Value of sub token is: \" << tok_new << std::endl);" << endl;
    ...
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter)
    {
	    ...
	    {  
               ...
               // indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Value of entity is: \" << entity_name << std::endl);" << endl;
    ...
    indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Checking when flow comes here \" << std::endl);" << endl;
    ...
    {   
        ...
        if (ftype->is_base_type())
        {
            ...
            if (btype->is_string() || btype->is_xml())
            {
                ...
                indent(out) << "curl_free(unescaped);" << endl;
                //indent(out) << "SANDESH_LOG(ERROR, __func__ << \"Checking when flow comes here before continue \" << std::endl);" << endl;
                indent(out) << "continue;" << endl;
