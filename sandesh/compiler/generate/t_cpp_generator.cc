...
void t_cpp_generator::generate_sandesh_session_log_unrolled_fn(ofstream &out,
    ...
    out << ";";
    // To be retained
    out << indent() << "static void LogUnrolled(std::string, " <<
        "SandeshLevel::type, const std::vector<SessionEndpoint> &, " <<
        "log4cplus::Logger &, log4cplus::Logger &, bool, bool);" << endl;
}

void t_cpp_generator::generate_sandesh_session_adjust_session_end_point_objects_fn(ofstream &out,
    t_sandesh *tsandesh) {
    out << "static void adjust_session_end_point_objects(bool " <<
          "is_send_sample_to_collector, bool " <<
         "is_send_slo_to_collector, std::vector<SessionEndpoint> & " <<
           "session_data);" << endl;
    out << endl;
}
...
void t_cpp_generator::generate_sandesh_flow_send_fn(ofstream &out,
 ...
 endl;
    ...
    if (((t_base_type *)t)->is_sandesh_session()) {
        /*
        out << indent() << "LogUnrolled(category, level, session_data, " <<
            "Sandesh::sample_logger(), Sandesh::slo_logger(), " <<
            "Sandesh::is_send_sampled_to_logger_enabled(), " <<
            "Sandesh::is_send_slo_to_logger_enabled()); " << endl;
       */
    }
    scope_down(out);
    out << indent() << "if (level >= SendingLevel()) {" << endl;
    ...
    out << indent() << " if (is_send_sampled_to_collector_enabled() != is_send_slo_to_collector_enabled()) {" << endl;
    indent_up();
        //out << indent() << "adjust_session_end_point_objects(is_send_sampled_to_collector_enabled(), is_send_slo_to_collector_enabled(), snh->session_data);";
    scope_down(out);
    out << endl;
    out << indent() << "snh->set_level(level);" << endl;
    out << indent() << "snh->set_category(category);" << endl;
