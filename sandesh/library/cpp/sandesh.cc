...
bool Sandesh::sample_to_collector_ = false;
bool Sandesh::slo_to_logger_ = false;
bool Sandesh::sample_to_logger_ = false;
...
void Sandesh::set_logger_appender(const std::string &file_name, long max_file_size,
    ...
    if (std::find(destn.begin(), destn.end(), "file") !=
        ...
        if (is_sample_logger) {
            Sandesh::sample_to_logger_ = true;
        } else {
...
void Sandesh::set_send_to_collector_flags(
    ...
    if (std::find(sample_destn.begin(), sample_destn.end(), "collector") !=
            sample_destn.end()) {
        Sandesh::sample_to_collector_ = true;
    }
}
