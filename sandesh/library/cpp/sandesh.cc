...
void Sandesh::set_sample_logger_appender(const std::string &file_name, long max_file_size,
    ...
    if (std::find(sample_destn.begin(), sample_destn.end(), "file") !=
        ...
        props.setProperty(LOG4CPLUS_TEXT("filename"), file_name);
        std::stringstream file_size_ss;
        file_size_ss << max_file_size;
        props.setProperty(LOG4CPLUS_TEXT("maxFileSize"),
                          file_size_ss.str());
        std::stringstream backup_index_ss;
        backup_index_ss << max_backup_index;
        props.setProperty(LOG4CPLUS_TEXT("maxBackupIndex"), backup_index_ss.str());
        props.setProperty(LOG4CPLUS_TEXT("useLockFile"),
                          std::string("true"));
        // Append file appender to SLO logger
        SharedAppenderPtr fileappender(new RollingFileAppender(props));
...
void Sandesh::set_send_2_logger_flags(
    const std::vector<std::string> &sample_destn) {
    if (std::find(sample_destn.begin(), sample_destn.end(), "file") !=
            sample_destn.end()) {
        Sandesh::sample_2_logger_ = true;
