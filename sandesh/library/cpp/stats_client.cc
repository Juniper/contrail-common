...
void StatsClientLocal::Initialize() {
    ...
    if (ec) {
        SANDESH_LOG(ERROR, "LOCAL could not connect to socket: " << ec);
    }
    is_connected_ = true;
}
...
size_t StatsClientLocal::SendBuf(uint8_t *data, size_t size) {
    ...
    if (ec) {
        SANDESH_LOG(ERROR, "LOCAL could not send to socket: " << ec);
        is_connected_ = false;
    }
    return ret;
}
...
void StatsClientRemote::Initialize() {
    ...
    if (ec) {
        SANDESH_LOG(ERROR, "UDP could not open socket: " << ec);
        return;
    }
    stats_socket_->connect(stats_server_ep_, ec);
    if (ec) {
        SANDESH_LOG(ERROR, "UDP could not connect address: " << ec);
        return;
    }
    is_connected_ = true;
}
...
size_t StatsClientRemote::SendBuf(uint8_t *data, size_t size) {
    ...
    size_t ret = stats_socket_->send(boost::asio::buffer(data, size), 0, ec);
    if (ec) {
        SANDESH_LOG(ERROR, "REMOTE could not send to socket: " << ec);
        is_connected_ = false;
    }
    return ret;
}
