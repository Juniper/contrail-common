...
class TJSONProtocol : public TVirtualProtocol<TJSONProtocol> {
 ...
 public:
  ...
  int32_t readJSON(std::string& str);
 ...
 private:
  ...
  int32_t readJSONSyntaxChar(uint8_t ch);

  int32_t readJSONSyntaxString(const std::string &str);

  int32_t readJSONString(std::string &str);

  int32_t readJSONTag(std::string &str, bool endTag = false);

  int32_t readJSONNumericChars(std::string &str);

  int32_t readJSONCDATA(std::string &str);

  template <typename NumberType>
  int32_t readJSONInteger(NumberType &num);

  int32_t readJSONDouble(double &num);

  int32_t readJSONUuid(boost::uuids::uuid &uuid);
  ...
  std::vector<contrail::sandesh::protocol::TType> field_type_;

  std::vector<std::string> collection_name_stack_;
  
  std::string roomKey_prefix_;

  bool name_field_;

  bool is_write_enabled_;
  std::vector<std::string> fields_to_be_written_;

  bool is_begin_field_with_tag_;

  bool is_map_val_string_;
