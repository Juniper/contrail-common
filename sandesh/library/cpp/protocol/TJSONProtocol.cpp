...
int32_t TJSONProtocol::writeStructBegin(const char* name) {
  ...
  /*
  if ( is_first_element_list_.size() > 0 &&
      !is_first_element_list_[is_first_element_list_.size()-1] &&
      (current_sandesh_context_[current_sandesh_context_.size()-1] != "SANDESH" &&
       current_sandesh_context_[current_sandesh_context_.size()-1] != "MAP" )) {
      json += ",";
  } else {
      if(is_first_element_list_.size() > 0 &&
         (current_sandesh_context_[current_sandesh_context_.size()-1] != "SANDESH" &&
         current_sandesh_context_[current_sandesh_context_.size()-1] != "MAP" )) {
          is_first_element_list_[is_first_element_list_.size()-1] = false;
      }
  }*/
  ...
  if (is_first_element_list_.size() > 0 &&
      !is_first_element_list_[is_first_element_list_.size()-1] &&
      current_sandesh_context_[current_sandesh_context_.size()-1] == "LIST") {
      json += ",";
  } else if (is_first_element_list_.size() > 0) {
      is_first_element_list_[is_first_element_list_.size()-1] = false;
  }
  current_sandesh_context_.push_back("STRUCT");
  is_first_element_list_.push_back(true);
...
int32_t TJSONProtocol::writeStructEnd() {
  ...
  is_first_element_list_.pop_back();
  if(is_first_element_list_.size() > 0) {
      is_first_element_list_[is_first_element_list_.size()-1] = false;
  }
...
int32_t TJSONProtocol::writeContainerElementBegin() {
  ...
  if (!is_first_element_list_[is_first_element_list_.size()-1]) {
      // could be in map or list context, print , only in case of map val
      if(current_sandesh_context_[current_sandesh_context_.size()-1] == "MAP") {
          if(!in_map_val_context_[in_map_val_context_.size()-1]) {
  ...
  if (current_sandesh_context_[current_sandesh_context_.size()-1] == "LIST" &&
      is_primitive_element_list_[is_primitive_element_list_.size()-1]) {
  ...
  if (current_sandesh_context_[current_sandesh_context_.size()-1] == "MAP") {
     if(is_primitive_element_list_[is_primitive_element_list_.size()-1]) {
         json+= "\"";
     }
  }
...
int32_t TJSONProtocol::writeContainerElementEnd() {
  ...
  if (current_sandesh_context_[current_sandesh_context_.size()-1] == "MAP") {
      ...
      if (in_map_val_context_[in_map_val_context_.size()-1]) {
          in_map_val_context_[in_map_val_context_.size()-1] = false;
      } else {
          // In map key context
          json += ":";
          if(is_map_val_primitive_[is_map_val_primitive_.size()-1]) {
              in_map_val_context_[in_map_val_context_.size()-1] = true;
          }
      }
  }
